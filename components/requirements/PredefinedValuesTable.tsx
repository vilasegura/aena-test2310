...
import { PredefinedValue } from '../../types';

type SortKey = keyof Omit<PredefinedValue, 'id' | 'attributeId'>;

const useSortableData = (items: PredefinedValue[], config: { key: SortKey; direction: string } | null = null) => {
...
const PredefinedValuesTable: React.FC<{ values: PredefinedValue[] }> = ({ values }) => {
    const { items, requestSort, sortConfig } = useSortableData(values, { key: 'valorAdmitido', direction: 'ascending'});
    ...
    const tableHeaders: { key: SortKey; label: string, className?: string }[] = [
        { key: 'valorAdmitido', label: 'Valores Admitidos' },
        { key: 'formato', label: 'Formato' },
        { key: 'unidad', label: 'Unidad' },
        { key: 'tipoEjemploRevit', label: 'Tipo/Ejemplo Revit' },
        { key: 'grupoAtributoRevit', label: 'Grupo atributo Revit' },
        { key: 'categoriaRevit', label: 'Categoria Revit' },
    ];
        ...
        <div className="overflow-x-auto border border-gray-200 rounded-lg">
            <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {items.map((value) => (
                        <tr key={value.id} className="hover:bg-gray-50 transition-colors duration-150">
                            <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-800">{value.valorAdmitido}</td>
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{value.formato}</td>
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{value.unidad}</td>
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{value.tipoEjemploRevit}</td>
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{value.grupoAtributoRevit}</td>
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{value.categoriaRevit}</td>
...
export default PredefinedValuesTable;